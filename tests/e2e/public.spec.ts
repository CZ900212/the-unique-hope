...
test("homepage shows the new T3-style workflow positioning", async ({ page }) => {
  ...
  await expect(page.locator("header").getByRole("link", { name: "Apply to Join" })).toBeVisible();
...
test("public auth routes render", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Create Your Tutor Account" })).toBeVisible();
  await expect(page.locator('input[name="name"]')).toBeVisible();
  await expect(page.getByLabel("Standardized Scores (SAT/TOEFL, etc.)")).toBeVisible();
  await expect(page.getByRole("button", { name: "Continue" })).toBeVisible();
...
test("reset password route without a token blocks submission and guides the user back", async ({
  ...
  await expect(
    page.getByText("Reset token is missing. Request a new password reset link."),
  ).toBeVisible();
...
test("student registration creates an account that can sign in and wait", async ({ page }) => {
  ...
  await page.getByLabel("Username").fill(`public_student_${unique}`);
  ...
  await page.getByLabel("Email or Username").fill(`public_student_${unique}`);
  await page.getByLabel("Password").fill("Public123456");
  await page.getByRole("button", { name: "Sign In" }).click();

  await expect(page.locator("h1", { hasText: "Waiting to be matched with a tutor" })).toBeVisible();
});

test("teacher registration creates an account that can sign in and wait", async ({ page }) => {
  const unique = Date.now().toString().slice(-6);
  ...
  await page.getByLabel("Standardized Scores (SAT/TOEFL, etc.)").fill("IELTS 7.5");
  await page.getByRole("button", { name: "Continue" }).click();
  await page.getByLabel("Username").fill(`public_teacher_${unique}`);
  await page.locator('input[name="password"]').fill("Public123456");
  await page.getByLabel("Confirm Password").fill("Public123456");
  await page.getByRole("button", { name: "Create Account" }).click();
  ...
  await page.goto("/login");
  await page.getByRole("button", { name: "Tutor" }).click();
  await page.getByLabel("Email or Username").fill(`public_teacher_${unique}`);
  await page.getByLabel("Password").fill("Public123456");
  await page.getByRole("button", { name: "Sign In" }).click();

  await expect(page.locator("h1", { hasText: "Waiting to be matched with a tutee" })).toBeVisible();
});

test("forgot password request fails safely when delivery is unavailable", async ({ page }) => {
  const unique = Date.now().toString().slice(-6);

  await page.goto("/forgot-password");
  await page.getByLabel("Email or Username").fill(`unknown_user_${unique}`);
  await page.getByRole("button", { name: "Send Reset Link" }).click();

  await expect(
    page.getByText("Password reset is not available for this account right now. Contact an admin."),
  ).toBeVisible();
});
