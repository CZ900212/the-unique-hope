...
type TeacherView = "lessons" | "booking";
...
export function TeacherDashboard() {
  ...
  const requestAppointment = api.teacher.requestAppointment.useMutation({
  ...
  const respondAppointment = api.teacher.respondAppointment.useMutation({
    onSuccess: async () => {
      await utils.teacher.dashboard.invalidate();
      setNotice({
        tone: "success",
  ...
  const currentLesson =
    lessons.find((lesson) => lesson.week_number === currentWeek) ?? null;
  ...
  useEffect(() => {
    setMeetingLink(matchedDashboard?.meetingLink ?? "");
  }, [matchedDashboard?.meetingLink]);
    ...
    startTransition(async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            payload.error?.message ?? messages.teacher.lessonSaveError,
          );
        }
        ...
        setNotice({
          tone: "success",
          text: messages.teacher.lessonSaved,
        });
      } catch (error) {
        setNotice({
          tone: "error",
          text: getDisplayErrorMessage(error, messages.teacher.lessonSaveError),
        });
      }
    });
  }
  ...
  function persistMeetingLink() {
    startTransition(async () => {
      ...
      try {
        ...
        setNotice({
          tone: "success",
          text: messages.teacher.meetingLinkSaved,
        });
      } catch (error) {
        setNotice({
          tone: "error",
          text: getDisplayErrorMessage(
            error,
            messages.teacher.meetingLinkSaveError,
          ),
        });
      }
    });
  ...
  const taughtCount = lessons.filter(
  ...
  return (
    <PortalShell
      ...
      navItems={[
        {
          label: messages.teacher.weeklyLessons,
        ...
        ...(isMatched
          ? [
              ...
              {
                label: messages.teacher.badgeGuidelines,
    ...
    >
      ...
      activeView === "lessons" ? (
        <>
          <div className="grid gap-6 xl:grid-cols-[1.08fr_0.92fr]">
            <section className="dash-card p-4 sm:p-6 lg:p-8">
              <h2 className="border-l-3 border-[var(--color-primary)] pl-4 text-3xl font-[var(--font-title)] text-[var(--color-text-main)]">
                {messages.teacher.lessonEvidence}
              </h2>
              ...
              <label
                ...
                className="group mt-6 grid min-h-56 w-full cursor-pointer place-items-center overflow-hidden rounded-[var(--radius-lg)] border-2 border-dashed border-[var(--color-primary)] bg-[var(--color-primary-50)]/50 transition-colors hover:bg-[var(--color-primary-50)] sm:h-72"
              >
                {selectedFileUrl || currentLesson?.evidenceUrl ? (
                  ...
                  <div className="text-center text-sm text-[var(--color-primary-dark)]">
                    <div className="mx-auto mb-2 text-3xl text-[var(--color-primary)]">
                      +
                    </div>
              ...
              <select
                ...
                onChange={(event) =>
                  setNotesVisibility(toVisibility(event.target.value))
                }
              ...
              >
                <option value="shared">
                  {messages.teacher.shareWithStudent}
                </option>
              ...
              <div className="mt-6">
                <button
                  ...
                  className="btn-primary w-auto px-6 py-3 text-sm font-semibold"
                >
                  {isPending
                    ? messages.teacher.saving
                    : messages.teacher.saveLesson}
                </button>
              </div>
            </section>
              ...
              <div className="dash-card p-4 sm:p-6 lg:p-8">
                ...
                <div className="mt-4 flex items-center gap-3">
                  ...
                  <span className="text-3xl font-[var(--font-title)] text-[var(--color-text-main)]">
                    {dashboardQuery.data?.student?.name ??
                      messages.teacher.noStudent}
                  </span>
                </div>
                <div className="mt-2 text-sm text-[var(--color-text-secondary)]">
                  {dashboardQuery.data?.student?.contact ??
                    messages.teacher.noContact}
                </div>
                <div className="mt-6 rounded-[var(--radius-md)] bg-[var(--color-bg-secondary)] px-5 py-5 text-sm text-[var(--color-text-secondary)]">
                  <strong className="text-[var(--color-text-main)]">
                    {messages.teacher.latestFeedback}
                  </strong>{" "}
                  <br className="mb-2" />
                  {dashboardQuery.data?.latestSharedFeedback?.text ??
                    messages.teacher.feedbackEmpty}
                </div>
              </div>

              <div className="dash-card p-4 sm:p-6 lg:p-8">
                <div className="text-xs font-bold tracking-[0.2em] text-[var(--color-primary)] uppercase">
                  {messages.teacher.meetingLink}
                </div>
                <textarea
                  value={meetingLink}
                  onChange={(event) => setMeetingLink(event.target.value)}
                  aria-label={messages.teacher.meetingLinkInputLabel}
                  placeholder={messages.teacher.meetingLinkPlaceholder}
                  className="form-control mt-4 min-h-28 resize-none"
                />
                <p className="mt-3 text-sm leading-6 text-[var(--color-text-secondary)]">
                  {messages.teacher.meetingLinkHelper}
                </p>
                <div className="mt-5 flex flex-wrap gap-3">
                  <button
                    type="button"
                    aria-label={`${messages.common.save} ${messages.teacher.meetingLinkInputLabel}`}
                  ...
                  <button
                    ...
                    onClick={() =>
                      navigator.clipboard.writeText(meetingLink.trim())
                    }
          ...
          {isGuidelinesOpen ? (
            <DashboardDialog
              title={messages.teacher.guidelinesTitle}
