...
describe("resolveSafeCallbackPath", () => {
  ...
  it("keeps internal relative paths", () => {
    expect(
      resolveSafeCallbackPath("/student?tab=progress", fallbackPath, origin),
    ).toBe("/student?tab=progress");
  });
  ...
  it("falls back for protocol-relative and malformed values", () => {
    expect(
      resolveSafeCallbackPath("//evil.example/phish", fallbackPath, origin),
    ).toBe(fallbackPath);
    expect(resolveSafeCallbackPath("http://[", fallbackPath, origin)).toBe(
      fallbackPath,
    );
  });

  it("falls back for empty strings and javascript payloads", () => {
    expect(resolveSafeCallbackPath("", fallbackPath, origin)).toBe(
      fallbackPath,
    );
    expect(
      resolveSafeCallbackPath("javascript:alert(1)", fallbackPath, origin),
    ).toBe(fallbackPath);
  });
});
